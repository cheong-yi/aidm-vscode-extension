...
import { Task, TaskStatus, TaskUpdateRequest } from "../types/tasks";
...
export class SimpleMCPServer {
  ...
  private handleToolsList(request: JSONRPCRequest): JSONRPCResponse {
    const tools: Tool[] = [
      // Legacy tool for backward compatibility with existing tests
      {
      ...
      // Task management tools
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            requirement: { type: "string", optional: true },
      ...
      {
        name: "tasks/refresh",
        description: "Refresh tasks from file",
        inputSchema: {
          type: "object",
          properties: {
      ...
      },
      {
        ...
        description: "Get or update task test results",
        inputSchema: {
          type: "object",
          properties: {
            taskId: { type: "string" },
            testStatus: { type: "object", optional: true },
          },
          required: ["taskId"],
        },
      },
  ...
  private async handleToolCall(
    ...
    try {
      ...
      switch (name) {
        ...
        case "tasks/refresh":
          return await this.handleTasksRefresh(request, args);
  ...
  private async handleTasksList(
    ...
    try {
          ...
          content: [
            {
              ...
              text: `Tasks list:\n${tasks.map(
                (task: Task) =>
                  `- ID: ${task.id}, Title: ${task.title}, Status: ${task.status}, Priority: ${task.priority || 'N/A'}, Assignee: ${task.assignee || 'N/A'}`
              ).join("\n")}`,
            },
          ],
        },
      ...
      console.error("Error handling tasks list:", error);
      return {
        jsonrpc: "2.0",
        result: {
          content: [
            {
              type: "text",
              text: `Error retrieving tasks list: ${
                error instanceof Error ? error.message : String(error)
              }`,
            },
          ],
      ...
      return {
        ...
        result: {
          content: [
            {
              type: "text",
              text: this.formatTaskDetailsForAI(task),
            },
          ],
        },
        id: request.id,
  ...
  /**
   * Handle tasks/update-status tool call
   */
  private async handleTasksUpdateStatus(
    ...
    try {
        ...
        return {
          ...
          result: {
            content: [
              {
                type: "text",
                text: "Task ID and new status are required",
              },
            ],
            isError: true,
          },
      ...
      const updatedTask = await this.taskStatusManager.updateTask(
        updateRequest
      );
      ...
      return {
        ...
        result: {
          content: [
            {
              type: "text",
              text: `Task status updated: ${updatedTask.taskId} -> ${updatedTask.newStatus}`,
            },
          ],
        },
        id: request.id,
      ...
      console.error("Error handling tasks update status:", error);
      return {
        jsonrpc: "2.0",
        result: {
          content: [
  ...
  /**
   * Handle tasks/refresh tool call
   */
  private async handleTasksRefresh(
    ...
    try {
        ...
        return {
          ...
          result: {
            content: [
              {
                type: "text",
                text: "File path is required for refresh",
              },
            ],
            isError: true,
          },
      ...
      return {
        ...
        result: {
          content: [
            {
              type: "text",
              text: `Tasks refreshed from ${filePath}.`,
            },
          ],
        },
        id: request.id,
      ...
      console.error("Error handling tasks refresh:", error);
      return {
        jsonrpc: "2.0",
        result: {
          content: [
  ...
  /**
   * Handle tasks/dependencies tool call
   */
  private async handleTasksDependencies(
    ...
    try {
      ...
      const dependencyGraph = this.taskStatusManager.getTaskDependencyGraph(taskId);
      if (!dependencyGraph) {
        return {
          jsonrpc: "2.0",
          result: {
            content: [
              {
                type: "text",
                text: `No dependency information found for task ${taskId}.`,
              },
            ],
          },
          id: request.id,
      ...
      return {
        ...
        result: {
          content: [
            {
              type: "text",
              text: `Dependencies for task ${taskId}:\nDependencies: ${dependencyGraph.dependencies.join(", ") || "None"}\nDependents: ${dependencyGraph.dependents.join(", ") || "None"}\nBlocked: ${dependencyGraph.isBlocked}`,
            },
          ],
        },
        id: request.id,
      ...
      console.error("Error handling tasks dependencies:", error);
      return {
        jsonrpc: "2.0",
        result: {
          content: [
            {
              type: "text",
              text: `Error retrieving task dependencies: ${
                error instanceof Error ? error.message : String(error)
              }`,
            },
          ],
  ...
  /**
   * Handle tasks/test-results tool call
   */
  private async handleTasksTestResults(
    ...
    try {
        ...
        return {
          ...
          result: {
            content: [
              {
                type: "text",
                text: `No test results found for task ${taskId}.`,
              },
            ],
          },
          id: request.id,
      ...
      return {
        ...
        result: {
          content: [
            {
              type: "text",
              text: `Test results for task ${taskId}:\nTotal Tests: ${task.testStatus.totalTests}\nPassed: ${task.testStatus.passedTests}\nFailed: ${task.testStatus.failedTests}\nCoverage: ${task.testStatus.coverage || 'N/A'}%`,
            },
          ],
        },
        id: request.id,
      ...
      console.error("Error handling tasks test results:", error);
      return {
        jsonrpc: "2.0",
        result: {
          content: [
            {
              type: "text",
              text: `Error retrieving task test results: ${
                error instanceof Error ? error.message : String(error)
              }`,
            },
          ],
  ...
  /**
   * Format task details for AI consumption
   */
  private formatTaskDetailsForAI(task: Task): string {
    let formatted = `# Task Details: ${task.title}\n\n`;

    formatted += `**ID:** ${task.id}\n`;
    formatted += `**Status:** ${task.status}\n`;
    formatted += `**Priority:** ${task.priority || "N/A"}\n`;
    formatted += `**Complexity:** ${task.complexity}\n`;
    formatted += `**Assignee:** ${task.assignee || "N/A"}\n`;
    formatted += `**Created:** ${new Date(task.createdDate).toLocaleDateString()}\n`;
    formatted += `**Last Modified:** ${new Date(task.lastModified).toLocaleDateString()}\n`;
    ...
    if (task.estimatedHours) {
      formatted += `**Estimated Hours:** ${task.estimatedHours}\n`;
    }

    if (task.actualHours) {
      formatted += `**Actual Hours:** ${task.actualHours}\n`;
    }

    if (task.testStatus) {
      formatted += `## Test Status\n`;
      formatted += `- Total Tests: ${task.testStatus.totalTests}\n`;
      formatted += `- Passed: ${task.testStatus.passedTests}\n`;
      formatted += `- Failed: ${task.testStatus.failedTests}\n`;
      if (task.testStatus.coverage) {
        formatted += `- Coverage: ${task.testStatus.coverage}%\n`;
      }
      if (task.testStatus.lastRunDate) {
        formatted += `- Last Run: ${new Date(task.testStatus.lastRunDate).toLocaleDateString()}\n`;
      }
      formatted += "\n";
    }
